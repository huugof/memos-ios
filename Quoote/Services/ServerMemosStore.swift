...
final class ServerMemosStore: ObservableObject {
    ...
    /// Fetches all pages sequentially. Used by Quoote so the full history is
    ...
    /// Fetches all pages, but only if the first page is stale. Used by Quoote on foreground resume.
